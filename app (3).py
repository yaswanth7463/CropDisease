...
with gr.Blocks(title="🌿 Crop Disease Detector — Anantapur", theme=gr.themes.Soft(), css=custom_css) as demo:
    ...
    <div class="app-header">
        ...
        <div class="app-header-subtitle">
            Weather-Aware Disease Diagnosis & Remedy Advisor for Anantapur, Andhra Pradesh 🇮🇳
        </div>
    </div>
    """)
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=350):
            gr.HTML('<div class="section-card"><h3 style="color: #2d5016; margin-top: 0;">📸 Image Upload</h3></div>')
            ...
            submit_btn = gr.Button(
                "🔍 Detect Disease & Get Advice",
                variant="primary",
                size="lg",
                elem_classes="button-submit"
            )
            ...
            <div class="tips-section">
                ...
                • 🍃 Fill frame with diseased leaf<br>
                • 🚫 Avoid blur or distant photos<br>
                • ✅ One leaf per photo<br>
                • 🎯 Show clear symptoms
            </div>
            """)

        # Right column - Results
        with gr.Column(scale=2.5, min_width=500):
            # Weather Panel
            with gr.Group(elem_classes="result-section"):
                gr.HTML('<h3 style="color: #1976d2; margin-top: 0;">🌤️ Live Weather Data</h3>')
                weather_output = gr.Textbox(
                    label="Current Conditions — Anantapur District",
                    lines=3,
                    interactive=False,
                    elem_classes="weather-panel"
                )

            # Disease Detection
            with gr.Group(elem_classes="result-section disease-result"):
                gr.HTML('<h3 style="color: #f57c00; margin-top: 0;">🦠 Disease Detection</h3>')
                disease_output = gr.Textbox(
                    label="Detected Disease",
                    interactive=False,
                    elem_classes="remedy-box"
                )

            # Description
            with gr.Group(elem_classes="result-section"):
                gr.HTML('<h3 style="color: #2d5016; margin-top: 0;">📋 Disease Details</h3>')
            ...
            # Severity
            with gr.Group(elem_classes="result-section"):
                gr.HTML('<h3 style="color: #c62828; margin-top: 0;">⚠️ Severity & Urgency</h3>')
    ...
    with gr.Row():
        with gr.Column():
            with gr.Group(elem_classes="result-section"):
                gr.HTML('<h3 style="color: #2d5016; margin-top: 0;">💊 Treatment Steps</h3>')
    ...
    with gr.Row():
        with gr.Column():
            with gr.Group(elem_classes="result-section"):
                gr.HTML('<h3 style="color: #1976d2; margin-top: 0;">🌦️ Weather-Adjusted Advice</h3>')
                advisory_output = gr.Textbox(
                    label="Smart Recommendations + Local Availability",
                    lines=12,
                    interactive=False,
    ...
    <div class="footer-section">
        <h3 style="color: #2d5016; margin-top: 0;">📞 Local Support Resources</h3>
        <p style="margin: 0.5rem 0;">
            <strong>⚠️ Disclaimer:</strong> Educational use only. Always confirm with a certified agronomist.<br>
            <strong>📍 KVK Reddipalli:</strong> <a href="https://www.kvkreddipalli-angrau.org" target="_blank">kvkreddipalli-angrau.org</a> | 📞 08554-255560<br>
            <strong>🌾 District Agriculture Office:</strong> 📞 08554-274400
        </p>
    </div>
    """)
