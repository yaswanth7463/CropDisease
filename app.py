...
# ─── Load Model ──────────────────────────────────────────────────────────────
def load_model():
    m = models.resnet50(weights=None)  # Updated from deprecated 'pretrained' parameter
    
    # Model output layer: ResNet-50 was trained with 16 disease classes
    m.fc = nn.Sequential(
        nn.Identity(), 
...
def predict(image):
    ...
    # 4. NOT A LEAF - Very low confidence detection
    if confidence < 0.40:
        return (
            f"🚫 Not a Leaf Image ({confidence*100:.1f}% confidence)",
            "The uploaded image does not appear to be a leaf. Please upload a clear photo of a leaf from one of the supported crops.",
            "❌ Invalid Image",
            "🌿 Upload a clear, well-lit photo of:\n• A single leaf\n• Without soil or other objects\n• In natural lighting\n• With visible disease symptoms (if analyzing disease)",
            wx_panel,
            "📝 The model is trained to recognize:\n\n✅ Leaf images from these crops:\nApple, Corn (Maize), Potato, Tomato, Pepper, Strawberry\n\n❌ Please do NOT upload:\n• Non-leaf objects\n• Fruits or whole plants\n• Blurry or distant photos\n• Multiple leaves mixed together\n• Photos with soil or background clutter"
        )

    # 5. Low confidence fallback - Image looks like a leaf but uncertain which disease
    if confidence < 0.75:
        return (
            f"⚠️ Uncertain Detection ({confidence*100:.1f}%)",
            "The image appears to be a leaf, but the model cannot confidently identify the specific disease. This could be:\n• A crop from the supported list but with an unrecognized disease\n• An image that's too blurry, distant, or poorly lit\n• A healthy leaf that doesn't match the model's training data well",
            "❓ Unsure",
            "🔄 Try these steps:\n• Take a clearer, closer photo\n• Ensure bright natural lighting\n• Focus on the affected area\n• Avoid shadows and reflections",
            wx_panel,
            "💡 For best results:\n• Fill the frame with the leaf\n• Show clear disease symptoms if analyzing disease\n• Use high-resolution photo\n• Take photo in daylight, not indoors under artificial light\n\nIf still uncertain, contact: 📞 KVK Reddipalli: 08554-255560"
        )

    # 6. Valid prediction - Confident detection
...
custom_theme = gr.themes.Soft(
    ...
    block_title_text_color="#27ae60",
    border_color_primary="#27ae60",
)

with gr.Blocks(
    title="🌿 Crop Disease Detector — Anantapur",
    theme=custom_theme,
    css="""
    ...
    .header-section h1 { margin: 10px 0; font-size: 2.5em; font-weight: bold; }
    .header-section p { margin: 5px 0; font-size: 1.1em; opacity: 0.95; }
    .section-box {
        background: white;
    ...
    <div class="header-section">
        ...
        <p><strong>Anantapur Edition</strong> — Intelligent Disease Diagnosis & Weather-Aware Remedy Advisory</p>
        <p style="margin-top: 10px; font-size: 0.95em;">For Anantapur, Andhra Pradesh, India 🇮🇳</p>
    </div>
    """)
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=350):
            gr.HTML("""<div class="section-title">📸 Upload & Analyze</div>""")
            ...
            gr.HTML("""<div class="section-title" style="margin-top: 20px;">📋 Tips for Best Results</div>""")
            
            with gr.Group():
                gr.Markdown("""
                <div class="tip-box">
                <strong>✅ Do This:</strong><br>
                • 📸 Use bright natural light<br>
                • 🍃 Fill frame with leaf<br>
                ...
                gr.Markdown("""
                <div class="tip-box">
                <strong>❌ Avoid This:</strong><br>
                • 🚫 Blurry or distant shots<br>
                • 🌑 Poor lighting<br>
                • 📐 Multiple leaves mixed<br>
                • 🌘 Shadows on leaf
                </div>
                """)
        ...
        with gr.Column(scale=1.5, min_width=400):
            ...
            # Treatment Advice
            gr.HTML("""<div class="section-title" style="margin-top: 20px;">💊 Treatment Plan</div>""")
    ...
    with gr.Row():
        with gr.Column():
            gr.HTML("""<div class="section-title">🌤️ Weather-Aware Guidance</div>""")
            
            with gr.Group():
                weather_output = gr.Textbox(
                    label="🌡️ Live Weather Data — Anantapur District",
                    interactive=False,
                    lines=3,
                    elem_classes="section-box"
                )
                ...
                advisory_output = gr.Textbox(
                    label="🌦️ Weather-Adjusted Advice & Local Chemical Availability",
                    interactive=False,
                    lines=10,
                    elem_classes="section-box"
                )
    ...
    with gr.Row():
        with gr.Column():
            ...
            <strong>🌾 Supported Crops ({len(SUPPORTED_CROPS)})</strong><br>
            <small>{supported_text}</small>
            </div>
            """)

    # Connect button to predict function
    submit_btn.click(
        fn=predict,
        inputs=image_input,
    ...
    <div class="footer-section">
        <h3 style="margin-top: 0; color: #27ae60;">📞 Support & Resources</h3>
        <p style="margin: 10px 0;">
            <strong>🌾 Anantapur Krishi Vigyan Kendra (KVK):</strong><br>
            <a href="https://www.kvkreddipalli-angrau.org" target="_blank">kvkreddipalli-angrau.org</a> 
            · 📞 <strong>08554-255560</strong>
        </p>
        <p style="margin: 10px 0;">
            <strong>🏛️ District Agriculture Office, Anantapur:</strong><br>
            📞 <strong>08554-274400</strong>
        </p>
        <hr style="margin: 15px 0; border: none; border-top: 1px solid #ddd;">
        <p style="font-size: 0.9em; color: #666; margin: 10px 0;">
            <strong>⚠️ Disclaimer:</strong> This tool is for educational purposes only.<br>
            Always verify results with a certified agronomist before making critical farming decisions.
        </p>
        <p style="font-size: 0.85em; color: #999; margin-top: 15px;">
            🌿 Built for Anantapur farmers with weather integration & local chemical guidance
        </p>
    </div>
    """)
